...
import { StreamSDKDefaultError } from "../models/errors/streamsdkdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | StreamSDKDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | StreamSDKDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new StreamSDKDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new StreamSDKDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
